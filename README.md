...
**Versão atual no código:** `2.99.100`  
...
A versão `2.99.95` permite reexecutar diretamente os códigos que falharam em uma execução histórica. A reexecução permanece vinculada à execução original e não cria uma nova pasta no histórico. Depois que os códigos forem reexecutados, o botão não reaparece para aqueles mesmos códigos naquela execução original.
