...
_REQUIRED_BASE_METHODS = (
    ...
    "_planilha_desenhar_borda",
)


def _validar_base_aplicacao():
    """Impede inicialização silenciosamente incompleta da base consolidada."""
    faltantes = [
        nome for nome in _REQUIRED_BASE_METHODS if not hasattr(App, nome)
    ]
    if faltantes:
        raise RuntimeError(
    ...
    if not getattr(App, "_patch_ui_aplicado", False):
        raise RuntimeError("A camada de compatibilidade não foi inicializada.")
...
def install_ui(App):
    ...
    App._planilha_virtual_29929_marker = SM_AUTOLAB_GRADE_VIRTUAL_29926
