...
class Resultados:
    ...
    def registrar_sucesso(self, numero, codigo):
        self.itens.append(ResultadoCodigo(numero, str(codigo), "Sucesso",
            horario=datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
    def registrar_erro(self, numero, codigo, erro):
        self.itens.append(ResultadoCodigo(numero, str(codigo), "Erro",
            erro=str(erro), horario=datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
    @property
