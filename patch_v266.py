...
def _excluir_historico_planilha_v266(self, item):
    ...
    itens = self._carregar_historico_planilhas()
    self._salvar_historico_planilhas([x for x in itens if str(x.get("id", "")) != ident])
...
def _limpar_historico_planilhas_v266(self):
    ...
    if not itens:
        ...
        return
    if not messagebox.askyesno("Limpar Arquivos", "Tem certeza que deseja apagar todos os arquivos do histórico?", parent=win):
        return
...
def _abrir_historico_planilha_v266(self):
    ...
    ctk.CTkButton(
        ...
        border_color=self.ERROR, text_color=self.ERROR, font=("Segoe UI", 11, "bold")
    ).pack(side="right")
    ctk.CTkLabel(win, text="Planilhas salvas nos últimos 30 dias.", text_color=self.SUBTEXT, font=("Segoe UI", 11)).pack(anchor="w", padx=20, pady=(0, 12))
    ...
    for item in itens:
        ...
        ctk.CTkButton(actions, text="Abrir", width=78, height=34, corner_radius=7, fg_color=self.ACCENT, hover_color=self.ACCENT_HOVER, text_color="#FFFFFF", font=("Segoe UI", 11, "bold"), command=lambda it=item: self._abrir_snapshot_historico(it)).pack(side="left", padx=(0, 6))
        ctk.CTkButton(actions, text="×", width=34, height=34, corner_radius=7, fg_color=self.CARD, hover_color=("#FDECEC", "#3A2424"), border_width=1, border_color=self.ERROR, text_color=self.ERROR, font=("Segoe UI", 16, "bold"), command=lambda it=item: self._excluir_historico_planilha(it)).pack(side="left")
...
def _aplicar_status_finalizado_v266(self, texto):
    ...
    try:
        ...
        self.status_pill.configure(fg_color=("#E7F5E7", "#21482A"))
        self.status_text.configure(text="Finalizado", text_color=self.SUCCESS, font=("Segoe UI", 13, "bold"))
        modo = ctk.get_appearance_mode().lower()
        self.status_indicator.configure(bg="#21482A" if modo == "dark" else "#E7F5E7")
        self._iniciar_pisca_status()
    except Exception:
        pass
    self._status_finalizado_job = self.app.after(10000, lambda: self.__v265_aplicar_status("Pronto"))
