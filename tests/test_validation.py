...
class ConsolidatedValidationTests(unittest.TestCase):
    ...
    def test_regressoes_visuais_e_de_rolagem_estao_protegidas(self):
        ...
        self.assertIn('janela.geometry(f"{largura}x{altura}+{x}+{y}")', interface)
        self.assertIn("app_x = self.app.winfo_rootx()", interface)
        self.assertIn("app_y = self.app.winfo_rooty()", interface)
        self.assertIn("def _centralizar_janela(self, janela, largura=None, altura=None):", interface)
        self.assertIn("self._centralizar_janela(popup, 560, 420)", interface)
        self.assertIn("self._centralizar_janela(win, 680, 500)", interface)
        self.assertIn("self._centralizar_janela(win, 1080, 720)", interface)
        self.assertIn("self._centralizar_janela(janela, 430, 165)", interface)
        self.assertIn("def _mostrar_confirmacao_atualizacao(self, info):", interface)
        self.assertIn("self._centralizar_janela(janela, 470, 230)", interface)
