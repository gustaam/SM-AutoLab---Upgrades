...
class CanonicalRuntimeTests(unittest.TestCase):
    ...
    def test_iniciar_has_one_canonical_text_owner(self):
        ...
        self.assertEqual(source.count("text=self.INICIAR_LABEL"), 1)
        self.assertEqual(source.count("text=icon_play"), 1)
        self.assertIn('icon_play = "\\ue768"', source)
    ...
    def test_linhas_do_historico_usam_layout_resumido(self):
        source=(self.root/"interface.py").read_text(encoding="utf-8")
    ...
    def test_linhas_do_historico_tem_dimensoes_responsivas(self):
        source=(self.root/"interface.py").read_text(encoding="utf-8")
        start=source.index("def _criar_pasta_historico")
        end=source.index("def _atualizar_visual_selecao_historico",start)
        block=source[start:end]
    ...
    def test_historico_usa_linha_resumida_sem_botao_detalhes(self):
        source=(self.root/"interface.py").read_text(encoding="utf-8")
        start=source.index("def _criar_pasta_historico")
        end=source.index("def _atualizar_visual_selecao_historico",start)
        block=source[start:end]
    ...
    def test_historico_linhas_mantem_selecao_e_hover(self):
        source = (self.root / "interface.py").read_text(encoding="utf-8")
        start = source.index("def _criar_pasta_historico")
        end = source.index("def _atualizar_visual_selecao_historico", start)
        block = source[start:end]
    ...
    def test_notificacao_do_historico_e_um_ponto_redondo_e_mais_a_direita(self):
        ...
        self.assertGreaterEqual(source.count("y=2"), 2)
    ...
    def test_history_linha_aceita_clique_unico_e_ctrl_multiseleciona(self):
        source=(self.root/"interface.py").read_text(encoding="utf-8")
        start=source.index("def _criar_pasta_historico")
