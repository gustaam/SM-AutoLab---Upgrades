...
def validate_workflow_security(root: Path) -> None:
    ...
    if (
        "github.event.workflow_run.head_sha == github.sha" not in release and
        '${{ github.event.workflow_run.head_sha }}' not in release
    ):
        fail("release.yml deve impedir publicação a partir de validação obsoleta da main")
